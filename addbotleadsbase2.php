...
//https://api.telegram.org/bot6742815092:AAER2AZyrPM3tKtlDfK5BSvBvMFM_ZUuUYw/setWebhook?url=https://crm.mkggroup.ru/addbotleadsbase2.php
$site_dir = dirname(dirname(__FILE__)).'/'; // корень сайта
$bot_token = '6742815092:AAER2AZyrPM3tKtlDfK5BSvBvMFM_ZUuUYw'; // токен вашего бота
...
$order_chat_id = '-1002022808065';  //chat_id менеджера компании для заявок
...
file_put_contents(__DIR__ . '/lmessage3.txt', print_r($data, true));
...
if (!empty($data['message']['text'])) {
    ...
    //if ($pos > 0) {
		...
		$i_date_create = date("Y-m-d");
			...
			$i_sql1 = 'INSERT INTO `bez_unprocessed_base` (`fio`, `phone_number`, `vopros`, `address`, `city`, `status`, `date_create`, `id_otdel`) VALUES ("' . $i_fio . '", "' . $i_phone_number . '", "' .  $i_vopros . '", "' . $i_address . '", "' . $i_city . '", "' . $i_status . '", "' . $i_date_create . '" , "2")';	
