...
//https://api.telegram.org/bot7474247897:AAH_nS_9HZponOcYm39ClTwcZvsWP8pk0aE/setWebhook?url=https://crm.mkggroup.ru/addbotleadsbase3.php
$site_dir = dirname(dirname(__FILE__)).'/'; // корень сайта
$bot_token = '7474247897:AAH_nS_9HZponOcYm39ClTwcZvsWP8pk0aE'; // токен вашего бота
...
$order_chat_id = '-1002246442133';  //chat_id менеджера компании для заявок
...
file_put_contents(__DIR__ . '/lmessage4.txt', print_r($data, true));
...
if (!empty($data['message']['text'])) {
    ...
    //if ($pos > 0) {
		...
		$i_date_create = date("Y-m-d");
			...
			$i_sql1 = 'INSERT INTO `bez_unprocessed_base` (`fio`, `phone_number`, `vopros`, `address`, `city`, `status`, `date_create`, `id_otdel`) VALUES ("' . $i_fio . '", "' . $i_phone_number . '", "' .  $i_vopros . '", "' . $i_address . '", "' . $i_city . '", "' . $i_status . '", "' . $i_date_create . '" , "3")';	
