...
//https://api.telegram.org/bot6058314242:AAHuTA7FOK57XWzOTRS1gqOsErd3BZNqROk/setWebhook?url=https://crm.mkggroup.ru/addbotleads.php
$site_dir = dirname(dirname(__FILE__)).'/'; // корень сайта
$bot_token = '6058314242:AAHuTA7FOK57XWzOTRS1gqOsErd3BZNqROk'; // токен вашего бота
...
$order_chat_id = '-1001902629180';  //chat_id менеджера компании для заявок
...
file_put_contents(__DIR__ . '/lmessage1.txt', print_r($data, true));
...
*/
    ...
    if ($pos > 0) {
		...
		if($cnt->getNumRows() > 0)
		{
			...
			$i_sql1 = 'INSERT INTO `bez_unprocessed` (`fio`, `phone_number`, `vopros`, `address`, `city`, `status`, `date_create`, `date_zagruzki`, `is_dubl`,`istochnik`) VALUES ("' . $i_fio . '", "' . $i_phone_number . '", "' .  $i_vopros . '", "' . $i_address . '", "' . $i_city . '", "' . $i_status . '", "' . $i_date_create . '", "' . $i_date_zagruzki . '", "N", "crm")';	
