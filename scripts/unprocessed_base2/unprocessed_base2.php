...
class UnprocessedBase2
{
    ...
    public function get_statuses() {
        $res = $this->db_connect->query("
            ...
            FROM
                {$this->pf}status
                WHERE {$this->pf}status.status_id IN(10, 11, 6, 8, 9, 17, 18, 19, 20, 5, 37)
    ...
    private function get_counters() {  # Вернет счетчики
        ...
        $date_filter = "({$this->pf}unprocessed_base2.status = 6 AND DATE({$this->pf}unprocessed_base2.date_time_status_change) = CURDATE() {$where_filter}) OR ({$this->pf}unprocessed_base2.status = 8 AND DATE({$this->pf}unprocessed_base2.date_create) = CURDATE() {$where_filter}) OR ({$this->pf}unprocessed_base2.status = 11 AND DATE({$this->pf}unprocessed_base2.date_create) = CURDATE() {$where_filter}) OR ({$this->pf}unprocessed_base2.status = 9 AND DATE({$this->pf}unprocessed_base2.date_create) = CURDATE() {$where_filter}) OR ({$this->pf}unprocessed_base2.status = 17 AND DATE({$this->pf}unprocessed_base2.date_create) = CURDATE() {$where_filter}) OR ({$this->pf}unprocessed_base2.status IN(10, 11, 6, 8, 9, 17, 18, 19, 20, 5) AND DATE({$this->pf}unprocessed_base2.date_create) = CURDATE() {$where_filter})";

        if((isset($this->data['is_search']) and $this->data['is_search'] === 'true') or $user_id !== '') {
            $date_filter = "({$this->pf}unprocessed_base2.status = 6 {$where_filter}) OR ({$this->pf}unprocessed_base2.status = 8 {$where_filter}) OR ({$this->pf}unprocessed_base2.status = 11 {$where_filter}) OR ({$this->pf}unprocessed_base2.status = 9 {$where_filter}) OR ({$this->pf}unprocessed_base2.status = 17 {$where_filter}) OR ({$this->pf}unprocessed_base2.status IN(10, 11, 6, 8, 9, 17, 18, 19, 20, 5) {$where_filter})";
        ...
        if(isset($this->data['date_start']) and isset($this->data['date_end'])) {
            ...
            $date_filter = "({$this->pf}unprocessed_base2.status = 6 AND DATE({$this->pf}unprocessed_base2.date_time_status_change) BETWEEN '{$ds}' AND '{$dend}' {$where_filter}) OR ({$this->pf}unprocessed_base2.status = 8 AND DATE({$this->pf}unprocessed_base2.date_create) BETWEEN '{$ds}' AND '{$dend}' {$where_filter}) OR ({$this->pf}unprocessed_base2.status = 11 AND DATE({$this->pf}unprocessed_base2.date_create) BETWEEN '{$ds}' AND '{$dend}' {$where_filter}) OR ({$this->pf}unprocessed_base2.status = 9 AND DATE({$this->pf}unprocessed_base2.date_create) BETWEEN '{$ds}' AND '{$dend}' {$where_filter}) OR ({$this->pf}unprocessed_base2.status = 17 AND DATE({$this->pf}unprocessed_base2.date_create) BETWEEN '{$ds}' AND '{$dend}' {$where_filter}) OR ({$this->pf}unprocessed_base2.status IN(10, 11, 6, 8, 9, 17, 18, 19, 20, 5) AND DATE({$this->pf}unprocessed_base2.date_create) BETWEEN '{$ds}' AND '{$dend}' {$where_filter})";
        ...
        $res = $this->db_connect->query("
            SELECT
                COUNT(CASE WHEN bez_unprocessed_base2.status = 6 {$date_filter_callings} THEN bez_unprocessed_base2.id END) AS callings,
                COUNT(CASE WHEN bez_unprocessed_base2.status = 8 THEN bez_unprocessed_base2.id END) AS non_calls,
                COUNT(CASE WHEN bez_unprocessed_base2.status = 11 THEN bez_unprocessed_base2.id END) AS rejections,
                COUNT(CASE WHEN bez_unprocessed_base2.status = 9 THEN bez_unprocessed_base2.id END) AS defects,
                COUNT(CASE WHEN bez_unprocessed_base2.status = 17 THEN bez_unprocessed_base2.id END) AS contracts,
                COUNT(CASE WHEN bez_unprocessed_base2.status IN(10, 11, 6, 8, 9, 17, 18, 19, 20, 5) {$date_filter_all_leads} THEN bez_unprocessed_base2.id END) AS all_leads
            FROM
    ...
    public function get_unprocessed_base() {
        ...
        if(isset($this->data['date_start']) and isset($this->data['date_end'])) {
            ...
            if(!empty($this->data['filter']['status']) and $this->data['filter']['status'] == 6) {
        ...
        $res = $this->db_connect->query("
            ...
            FROM
                {$this->pf}unprocessed_base2
            LEFT JOIN
                {$this->pf}status ON {$this->pf}status.status_id = {$this->pf}unprocessed_base2.status
            LEFT JOIN
            ...
            WHERE
                {$this->pf}status.status_id IN(10, 11, 6, 8, 9, 17, 18, 19, 20, 5, 37) {$where_filter}
            GROUP BY 
                {$this->pf}unprocessed_base2.id        
            ORDER BY 
                {$order_by}        
        ...
        if($unprocessed_base_requests != NULL) {
            if(!empty($this->data['view_name'])) {
                $path = $_SERVER['DOCUMENT_ROOT'] . '/scripts/unprocessed_base2/view/' . $this->data['view_name'] . '.php';
                include ($path);
            } else {
