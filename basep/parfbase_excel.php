...
foreach($data as $row) {
	...
	if($no_check_duplicates) {
		$info_by_number = find_info_by_number($clean_phone); //get_city_group($region_request);
		$no_operator = $info_by_number['operator_name'];
		$no_region = $info_by_number['region_name'];
		if(trim($i_fio) == "") {
			$i_fio = $no_region;
		}
		if($info_by_number['city_id'] == 114) {
			$phones_without_region[] = ['phone_number' => $clean_phone, 'fio' => $i_fio, 'source' => $source_request, 'vopros' => $comment, 'operator_name' => $no_operator, 'region_name' => $no_region]; 
		}
		$group_requests = get_group_request($source_request, count($data));	
		$i_sqld = 'INSERT INTO bez_unprocessed_base_excel
	...
	if($no_check_duplicates) {
		...
		$group_requests = get_group_request($source_request, count($data));	
		$double_phones = is_dublicate($clean_phone,$source_request,$group_requests);
...
function find_info_by_number($phone_number) {
	global $db_connect;

	$res = $db_connect->query('SELECT region_name, mobile_operator
								FROM def_codes 
								WHERE LEFT("' . $phone_number . '", 3) = defcode AND RIGHT("' . $phone_number . '", 7) BETWEEN from_code AND to_code;');

	if ($res->getNumRows() > 0) {
		$row = $res->fetchAssoc();
		$region_name = $row['region_name'];		
		$operator_legal_name = $row['mobile_operator'];
	} else {
		$region_name = 'Регион не определен';
		$operator_legal_name = 'Оператор не определен';
	}

	$city_id = get_city_group($region_name);
	$operator = get_operator($operator_legal_name);
	$operator_id = $operator['operator_id'];
	$operator_name = $operator['operator_name'];
	return ['city_id' => $city_id, 'region_name' => $region_name, 'operator_id' => $operator_id, 'operator_name' => $operator_name];	

}

function is_dublicate($clean_phone,$source_request,$group_requests) {
	global $db_connect;
	...
	$query = "
		with wt_data_0 as (
			SELECT f.phone_number, max(f.source) as source, f.status_name, f.date_time, f.operator_name, f.region_name, f.hold_status_name
			  FROM (
				  SELECT phone_number, source, IFNULL(s1.status_name, 'Не назначен') AS status_name, CASE WHEN status = 6 OR status = 15 THEN DATE(date_time_status_change) WHEN status = 11 OR status = 30 OR status = 22 OR status = 21 OR status = 24 OR status = 23 OR status = 25 OR status = 26 OR status = 28 OR status = 8 OR status = 31 OR status = 27 OR status = 29 OR status = 32 OR status = 36 THEN DATE(date_time_of_last_save) END AS date_time, mobile_operators.operator_name AS operator_name, bez_cities_group.name AS region_name, IFNULL(s2.status_name, '') AS hold_status_name
                    FROM bez_unprocessed_base
                  	LEFT JOIN bez_status s1
                    ON bez_unprocessed_base.status = s1.status_id
                    LEFT JOIN bez_status s2
                    ON bez_unprocessed_base.hold_status_id = s2.status_id
                    LEFT JOIN mobile_operators
                    ON bez_unprocessed_base.mobile_operator_id = mobile_operators.operator_id
                    LEFT JOIN bez_cities_group
					ON bez_unprocessed_base.auto_city_group = bez_cities_group.id
				   UNION ALL
				  SELECT phone_number, source, IFNULL(s1.status_name, 'Не назначен') AS status_name, CASE WHEN status = 6 OR status = 15 THEN DATE(date_time_status_change) WHEN status = 11 OR status = 30 OR status = 22 OR status = 21 OR status = 24 OR status = 23 OR status = 25 OR status = 26 OR status = 28 OR status = 8 OR status = 31 OR status = 27 OR status = 29 OR status = 32 OR status = 36 THEN DATE(date_time_of_last_save) END AS date_time, mobile_operators.operator_name AS operator_name, bez_cities_group.name AS region_name, '' AS hold_status_name 
                    FROM bez_unprocessed_base_excel
                    LEFT JOIN bez_status s1
                    ON bez_unprocessed_base_excel.status = s1.status_id
                    LEFT JOIN mobile_operators
                    ON bez_unprocessed_base_excel.mobile_operator_id = mobile_operators.operator_id
                    LEFT JOIN bez_cities_group
					ON bez_unprocessed_base_excel.auto_city_group = bez_cities_group.id
				) f	
               GROUP BY f.phone_number
               ORDER BY CASE WHEN f.date_time IS NULL THEN 0 ELSE 1 END DESC, f.date_time DESC
			), wt_data as (
	  ...
	  SELECT bd.fio, bd.phone_number as phone_number, b.source, bd.vopros, bd.city, b.status_name, IFNULL(b.date_time, '') AS date_time, IFNULL(b.operator_name, '') AS operator_name, IFNULL(b.region_name, '') AS region_name, hold_status_name 
		FROM wt_data bd
		JOIN wt_data_0 b
		  on CASE
			{$filter}
...
function is_unique($clean_phone,$source_request,$group_requests) {
	global $db_connect;
...
<?php 
	if(count($unique_data) !== 0) {
		...
		foreach($unique_data as $row){ 
			$lineData = array(trim('8'.$row['phone_number']), trim($row['fio']), trim($row['city']), trim($row['vopros']), trim($row['operator_name']), trim($row['region_name'])); 
	...
	if(!$no_check_duplicates and count($double_phones) !== 0) {
		$delimiter2 = ";"; 
		$buffer2 = fopen(__DIR__ . '/uploads/double_data.csv', 'w'); 
		// Добавляет в начало файла метку BOM, благодаря этому файл откроется в Excel с нормальной кодировкой.
		fputs($buffer2, chr(0xEF) . chr(0xBB) . chr(0xBF));
		// Данные в файл csv
		foreach($double_phones as $row){ 
			$lineData2 = array(trim('8'.$row['phone_number']), trim($row['fio']), trim($row['source']), trim($row['city']), trim($row['vopros']), trim($row['status_name']), trim($row['date_time']), trim($row['operator_name']), trim($row['region_name']), trim($row['hold_status_name'])); 
			fputcsv($buffer2, $lineData2, $delimiter2); 
		} 
		fclose($buffer2); 
		//echo '<script>location.replace("https://crm.mkggroup.ru/basep/uploads/double_data.csv");</script>';
		//exit();
	}
...
<?php } 
if(count($phones_without_region) !== 0) {?>
  <input id="tab-btn-3" name="tab-btn" type="radio" value="">
  <label for="tab-btn-3">Регион не определен</label>
<?php } 
if(!$no_check_duplicates and count($double_phones) !== 0) {?>  
  <div class="tab-content" id="content-1">
	<p><a href="/basep/uploads/double_data.csv" download="double_data.csv" > <span> скачать дубликаты </span></a></p>
	<table>
		<thead>
			<tr>
				<th>Номер</th>
...
if(count($unique_data) !== 0) {?>
  ...
  <p><a href="/basep/uploads/unique_data.csv" download="unique_data.csv" > <span> скачать уникальные </span></a></p>
	<table>
		<thead>
			<tr>
				<th>Номер</th>
...
if(count($phones_without_region) !== 0) {?>
  <div class="tab-content" id="content-3">
	<table>
		...
		<tbody>
			<?php foreach($phones_without_region as $row): ?>
				<tr>
					<td>
						<?= $row['phone_number']; ?>
					</td>
