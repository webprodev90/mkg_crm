...
foreach($data as $row) {
	...
	if($no_check_duplicates) {
		...
		$i_sqld = 'INSERT INTO bez_unprocessed_base
			(fio, phone_number, vopros, city, status, source, date_create, city_group, manual, group_source)
			VALUES (
			"' . $i_fio . '", "' . $clean_phone . '", "' . $comment . '",
			"' . $i_city . '", "10", "' . $source_request . '", "' . $i_date_create . '",
			"' . $city_group . '", "r", "' . $group_requests . '"
		)';		
...
<?php 
	if(count($unique_data) !== 0) {
		...
		fclose($buffer); 
		echo '<script>location.replace("https://crmdev.mkggroup.ru/basep/uploads/unique_data.csv");</script>';
		exit();
	}
