...
//https://api.telegram.org/bot6337444900:AAH3P-GtXocQJxhd-LNEoR4DP8FTTcXNzYg/setWebhook?url=https://crm.mkggroup.ru/addbotleadsbase.php
$site_dir = dirname(dirname(__FILE__)).'/'; // корень сайта
$bot_token = '6337444900:AAH3P-GtXocQJxhd-LNEoR4DP8FTTcXNzYg'; // токен вашего бота
...
$order_chat_id = '-1001849617188';  //chat_id менеджера компании для заявок
...
file_put_contents(__DIR__ . '/lmessage2.txt', print_r($data, true));
...
if (!empty($data['message']['text'])) {
	...
	$r1p = substr($data['message']['text'],$pos_r1p + 5, $pos_r2p - $pos_r1p - 5);	
    ...
    //if ($pos > 0) {
		...
		$i_date_create = date("Y-m-d");
			...
			$i_sql1 = 'INSERT INTO `bez_unprocessed_base` (`fio`, `phone_number`, `vopros`, `address`, `city`, `status`, `date_create`, `id_otdel`) VALUES ("' . $i_fio . '", "' . $i_phone_number . '", "' .  $i_vopros . '", "' . $i_address . '", "' . $i_city . '", "' . $i_status . '", "' . $i_date_create . '", "1")';	
